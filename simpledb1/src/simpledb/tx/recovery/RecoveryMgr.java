...
public class RecoveryMgr{
	...
	public static void checkpointWrite(){
		new CheckpointRecord().writeToLog();
	}

	public static void nQcheckpointWrite(List<Integer> list){
		new NQCheckpointRecord(list).writeToLog();
	}

	public static void endNQCKPTWrite(){
		new NQEndCkptRecord().writeToLog();
	}
	...
	public static void recover2(int earlier){
		Collection<Integer> finishedTxs = new ArrayList<Integer>();
		Iterator<LogRecord> iter = new LogRecordIterator();
		boolean endGordunmu = false;
		boolean endGormedenNQCKPTgorme = false;
		while(iter.hasNext()){
			LogRecord rec = iter.next();
			if(rec.op() == ENDNQCKPT){
				endGordunmu = true;
			...
			if(rec.op() == NQCHECKPOINT){
				if(endGordunmu == true){
					...
					endGormedenNQCKPTgorme = true;
				}
			}
			if((rec.txNumber() == earlier) && (endGormedenNQCKPTgorme == true))
				return;
			if(rec.op() == COMMIT || rec.op() == ROLLBACK)
				finishedTxs.add(rec.txNumber());
			else if(!finishedTxs.contains(rec.txNumber()))
