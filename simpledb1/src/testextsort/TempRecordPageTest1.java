...
public class TempRecordPageTest1 {
	...
	public static void main(String[] args) {
		...
		TablePlan planS=new TablePlan("student",tx);
		TableScan src=(TableScan) planS.open();
		src.beforeFirst();
		src.next();
		while (trp.insertFromScan(src)) // copy data from src to temp area
			if (!src.next()) {
				break;
			}
		trp.internalSort(sf);
		...
		//test #4: internal sort only a page
//		System.out
//				.println("**********test #4: page içerindeki kayýtlarý sýralama ***************");
//		trp = new TempRecordPage(ti, 0, tx);
//		trp.internalSort(sf); // DANGER: This modifies the record order of
//								// original file data. ROLLBACK at the end.
//		trp.printPage();
//		trp.close();
